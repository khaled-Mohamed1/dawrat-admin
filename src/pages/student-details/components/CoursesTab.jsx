...
const CoursesTab = ({ studentId }) => {
    ...
    const [isFavoritesLoading, setIsFavoritesLoading] = useState(true);
        ...
        // State for Course Demands
    const [demands, setDemands] = useState([]);
    const [demandsPagination, setDemandsPagination] = useState(null);
    const [demandsPage, setDemandsPage] = useState(1);
    const [isDemandsLoading, setIsDemandsLoading] = useState(true);

    // Fetch enrollments when the component loads or when the page changes
    const fetchEnrollments = useCallback(async () => {
        if (!studentId) return;
        setIsLoading(true);
        try {
            const response = await getStudentEnrollments(studentId, currentPage);
            if (response.success) {
    ...
    const fetchFavorites = useCallback(async () => {
        ...
        try {
            const response = await getStudentFavorites(studentId, favoritesPage);
            if (response.success) {
                // The actual course data is nested in the 'favorable' object
                setFavorites(response.data.map(fav => fav.favorable));
                setFavoritesPagination(response.meta);
            }
        } catch (error) {
    ...
    }, [studentId, favoritesPage]);

    const fetchDemands = useCallback(async () => {
        if (!studentId) return;
        setIsDemandsLoading(true);
        try {
            const response = await getStudentCourseDemands(studentId, demandsPage);
            if (response.success) {
                setDemands(response.data);
                setDemandsPagination(response.meta);
            }
        } catch (error) {
            console.error("Failed to fetch course demands", error);
        } finally {
            setIsDemandsLoading(false);
        }
    }, [studentId, demandsPage]);

    // Main useEffect to trigger fetching based on active tab
    useEffect(() => {
    ...
    }, [activeSection, fetchEnrollments, fetchFavorites, fetchDemands]);
                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-4">
                            {Array.from({ length: 3 }).map((_, i) => (
                                <div key={i} className="bg-muted/30 rounded-lg p-4 h-28 animate-pulse" />
                            ))}
                        </div>
                    ) : enrollments.length === 0 ? (
                        <div className="text-center py-8">
                            <Icon name="BookOpen" size={48} className="text-muted-foreground mx-auto mb-4" />
                            <p className="text-muted-foreground">No current enrollments found.</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
...
{activeSection === 'demands' && (
                <div>
                    {isDemandsLoading ? (
                        <div className="space-y-4">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="bg-muted/30 rounded-lg p-4 h-24 animate-pulse" />)}</div>
                    ) : demands.length === 0 ? (
                        <div className="text-center py-8">
                            <Icon name="MessageSquare" size={48} className="text-muted-foreground mx-auto mb-4" />
                            <p className="text-muted-foreground">This student has not submitted any course demands.</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {demands.map(demand => (
                    ...
                    <div className="mt-6">
                        <Pagination meta={demandsPagination} onPageChange={setDemandsPage} />
                    </div>
                </div>
            )}
            ...
            {activeSection === 'favorites' && (
                <div>
                    {isFavoritesLoading ? (
                        <div className="space-y-4">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="bg-muted/30 rounded-lg p-4 h-28 animate-pulse" />)}</div>
                    ) : favorites.length === 0 ? (
                        <div className="text-center py-8"><Icon name="Heart" size={48} className="text-muted-foreground mx-auto mb-4" /><p className="text-muted-foreground">No favorite courses found.</p></div>
                    ) : (
                        <div className="space-y-4">
                    ...
                    <div className="mt-6">
                        <Pagination meta={favoritesPagination} onPageChange={setFavoritesPage} />
                    </div>
                </div>
            )}
        </div>
