...
const ResetPasswordModal = ({ isOpen, onClose, student, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await resetUserPassword(student.user_id, newPassword);
            onSuccess(`Password for ${student.name} has been reset successfully.`);
            onClose();
            setNewPassword('');
        } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-card rounded-lg shadow-xl w-full max-w-md p-6">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                    Enter a new password for <span className="font-medium text-foreground">{student.name}</span>.
                </p>
